...
// create a variable to store the url from where we fetch the products
...
function initialize() {
    ...
    function selectCategory(e) {
        console.log('hello from selectCategory')
        ...
        if (category.value === lastCategory && searchTerm.value.trim() === lastSearch) {
            ...
            if (category.value == 'All') {
                ...
                products.forEach(e => {
                    if (e.type === lowerCaseType) {
                        categoryGroup.push(e);
                    }
                });
    ...
    function selectProducts() {
        console.log('hello from selectProducts');
        ...
        if (searchTerm.value.trim() === '') {
            ...
            categoryGroup.forEach(obj => {
                if (obj.name.indexOf(lowerCaseSearchTerm) !== -1) {
                    finalGroup.push(obj);
                }
            });
    ...
    function updateDisplay() {
        ...
        if (finalGroup.length === 0) {
            ...
            para.textContent = 'No elements to display!';
            main.appendChild(para);
        } else {
            // for each product we want to display, pass its product object to fetchBlob()
            finalGroup.forEach(obj => fetchBlob(obj));
        }
    };
    ...
    // display it
    function fetchBlob(elem) {
        console.log('hello from fetchBlob')
        // construct the URL path to the image file from the product.image property
        const url = 'images/' + elem.image;
        ...
        fetch(url).then(res => res.blob().then(blob => {
            ...
            // invoke showProduct
            showProduct(objectURL, elem);
        })).catch(err => console.log('Fetch problem: ' + err.message));
    };

    // Display a product inside the <main> element
    function showProduct(objectUrl, product) {
        console.log('hellof from showProduct')
